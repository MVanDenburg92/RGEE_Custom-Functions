...
interpolate_DF <- TsPP(evis_df2, tsgf=TSGFspline)
plot(Yt_interpolate)
...
# evi <-  lapply(nrow(evis_df3), function(x){
#   pixelly[[i]] <- evis_df3 %>%
#   arrange() %>% 
#   t()
# })
...
#just 2000
x <- TsPP(EVIseries, interpolate=TRUE)[1:365]
plot(x)
...
# nrow(rast) <- 20
# rast2 <- rasterize(DF_phenmet_sf$sos, rast,  fun=median) 
