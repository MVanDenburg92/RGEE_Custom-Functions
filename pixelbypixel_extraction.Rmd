...
#create time series 
NDVIseries <- ts(NDVIseries, start=c(2000, 1), end=c(2007, 12), frequency=12)
yt <- NDVIseries

# time series pre-processing ---interpolating 
Yt_interpolate <- TsPP(yt, tsgf=TSGFspline)
plot(Yt_interpolate)
...
y<-data.frame(Date=(seq(as.Date('2000/01/01'),as.Date('2007/01/01'), by="year")))   #create the year column
library(lubridate)
y$Year<-year(y$Date)
years <- y$Year
...
#Dataframe
DF <- data.frame(coords, sos, eos, los, pop) %>% 
  mutate(`year` = years)
