...
eco_mask <- st_read('Ecoregion_Small.shp',quiet = TRUE) %>% st_transform(4326) 
...
s3 <-  s3$filterDate(ee$Date('1999-12-01'), ee$Date('2002-01-01'))$filterBounds(eco_mask_ee)
...
evis_df2 <- data.frame(x = lng3,y = lats3,lapply(evis, "length<-", max(lengths(evis))))

#check if all values are numeric 
evis_df2 %>% mutate_if(purrr::compose(all,check.numeric),as.numeric)

library(raster)
XYZ_SP <- rasterFromXYZ(evis_df2, crs = '+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs')
brick(XYZ_SP)
...
Phenology 
```{r}
phenmapsmall <- PhenologyRaster(XYZ_SP, start=c(2000, 1), freq=12, tsgf="TSGFspline", approach="Deriv", checkseasonality = NULL)
PhenologyRaster(XYZ_SP, start = c(2005, 1), freq = 12,  tsgf = "TSGFspline", interpolate = TRUE, check.seasonality = NULL)
