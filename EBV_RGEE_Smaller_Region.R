...
mmm <- ee$Array(s4_img_list[[1]]$get("latitude")) %>% ee$Array$getInfo()
...
e <- seq(nimages2$getInfo())
evi_values4 <- list()
evi_values5 <- lapply(e,function(x){
  evi_values4[[1]] <-  np$array((ee$List(s4_img_list[[1]]$get(s4_names[[1]]) %>% ee$List$getInfo())))
  
})

evi_values5 <- lapply(e,function(x){
evi_values4[[index]] <-  ee$List(s4_img_list[[x]]$get(s4_names[[x]]))$getInfo()
})
...
evi_values5 <- lapply(evi_values5,function(x){
  np$array((x))
})
...
mmm <- np$array((ee$Array(s4_img_list[[1]]$get("latitude")) %>% ee$Array$getInfo()))

lats2 <-  np$array((ee$Array(s4_img_list[[1]]$get("latitude"))))
...
mynewlist = evi_values4$map(ee_utils_pyfunc(function(x) {ee$List(s2_img_list[[x]]$get(s2_names[[x]]))$getInfo()}
  )
)
